...
- 04-project-dialogs: Editor home updated with heading ("Create a project or open an existing one"), description, and New Project button with Plus icon. Three dialogs created: CreateProjectDialog (name input with live slug preview), RenameProjectDialog (prefilled auto-focused input, Enter submits, current name in description), DeleteProjectDialog (destructive confirmation, no input). `useProjectDialogs` hook manages dialog/form/loading state and mock project CRUD. ProjectSidebar updated to render owned projects with rename/delete actions (Pencil/Trash2 icons, hover-reveal), shared projects with no actions. Shared `Project` interface in `lib/types.ts`. Mock data: 2 owned + 1 shared project. Sidebar New Project button, editor home New Project button, and sidebar item actions all wired to correct dialogs. Mobile backdrop scrim closes sidebar on tap. `npm run build` passes.
- 05-prisma: Prisma schema models (`Project`, `ProjectCollaborator`, `ProjectStatus` enum) defined in `prisma/models/project.prisma`. Single cached Prisma client singleton implemented in `lib/prisma.ts`, branching between Accelerate (`prisma+postgres://`) and direct `@prisma/adapter-pg`. Configured `prisma.config.ts` to load `.env.local` followed by `.env`. First database migration (`20260728023115_init`) created and applied to database, client generated to `@/app/generated/prisma`. `npm run build` passes.
